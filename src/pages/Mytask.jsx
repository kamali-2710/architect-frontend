        ...
        {tasks.length > 0 ? (
          tasks.map((item) => (
            <Card key={item._id}>
              <div
                className={`task-card
                  ${item.status === "COMPLETED" ? "completed-task" : ""}
                  ${item.status === "REJECTED" ? "rejected-task" : ""}
                  ${item.status === "ASSIGNED" ? "assigned-task" : ""}`}
              >
                {/* STATUS BADGE */}
                <span className={`task-badge ${item.status.toLowerCase()}`}>
                  {item.status}
                </span>
                ...
                {/* UPLOAD / REWORK BUTTON (ONLY CHANGE ADDED) */}
                ...
                {item.status === "REJECTED" && (
                  <p className="reject-msg">
                    ❌ Rejected by Admin - Needs Rework
                  </p>
                )}
              </div>
            </Card>
